...
`/admin` 회원 명단에도 **팀 칸**이 붙습니다. 그리고 거기서 고치는 「월 한도」의
뜻이 바뀌었다는 것 — 팀에 든 사람에게는 **팀 잔량 안에서의 천장**이라는 것 —
을 화면이 말합니다. 안 그러면 운영자가 한도를 올렸는데 왜 막히는지 모릅니다.
...
| `app/admin/page.tsx` | 회원 명단에 팀 칸, 월 한도 설명 | 안 겹침 |
...
- **`/admin` 은 로컬 모드에서 500 입니다.** `createSupabaseAdminClient()` 를
  안전장치 없이 부릅니다. 팀 작업이 만든 문제가 아니고, 고치려면 그 화면의
  질의 여덟 개를 전부 손봐야 해서 남겨 뒀습니다. **그래서 관리자 화면의 팀 칸은
  눈으로 확인하지 못했습니다** — 표의 머리와 몸이 8 대 8 로 맞는 것만 세었습니다
