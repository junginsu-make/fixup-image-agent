...
**실호출 재검증이 아직 안 됐다.** 잘림의 원인은 제거했지만 32768 로 충분한지는
`docs/bugs/pdp-validation/w3-live-planning.cjs` 를 한 번 더 돌려야 안다(이미지 1회 $1 수준).
텍스트 모드가 통과하면 W3 를 닫고, 또 잘리면 종료 사유가 이제 그 자리에서 뜬다.
