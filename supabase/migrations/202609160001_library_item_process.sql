...
-- ── 배포보다 **먼저** 실행한다 ────────────────────────────────────
--
-- 코드가 이 칸을 쓰는데 칸이 없으면 PostgREST 가 `PGRST204` 로 거절한다.
-- 값을 `null` 로 보내도 마찬가지다 — 칸 이름 자체가 스키마 캐시에 없다.
-- 그러면 상세페이지·리디자인·캐릭터 저장이 **전부** 실패하고, 읽기도
-- `select("...,data")` 에서 같이 죽는다. 「나중에 해도 되는 것」이 아니다.
