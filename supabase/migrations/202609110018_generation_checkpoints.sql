begin;
drop function public.checkpoint_generation_run(uuid,uuid,jsonb,text,integer);
create function public.checkpoint_generation_run(p_id uuid,p_token uuid,p_checkpoint jsonb,p_state text,p_delay_seconds integer default 5,p_release boolean default true) returns jsonb
...
begin
  ...
  update public.generation_runs set checkpoint=p_checkpoint,state=p_state,updated_at=now(),
    ...
    lease_until=case when not p_release or p_state='settlement_pending' then lease_until else now() end
    where id=p_id returning * into r;
  if r.resource_type='sns' and p_checkpoint ? 'flow' then
    update public.sns_projects set data=jsonb_set(data,'{executionFlow}',p_checkpoint->'flow'),status='generating',updated_at=now()
      where id=r.resource_id and user_id=r.user_id;
  end if;
  return to_jsonb(r);
end $$;
revoke all on function public.checkpoint_generation_run(uuid,uuid,jsonb,text,integer,boolean) from public,anon,authenticated;
grant execute on function public.checkpoint_generation_run(uuid,uuid,jsonb,text,integer,boolean) to service_role;
