 ...
 */
/**
 * **읽은 연출은 쓸 수 있는 재료다** (2026-09-18 실측).
 *
 * 붙인 그림을 통째로 읽게 되면서 「손 여섯이 스마트폰으로 인물을 둘러싸 촬영」이
 * 기획 프롬프트에 실렸다. 그런데 기획은 그것을 **쓰지 않았다** — 오히려
 * `forbidden` 에 「스마트폰, 여러 손·팔」을 적어 금지했다.
 *
 * 이름표를 빼고 다시 재 봤더니 금지는 사라졌지만 어느 그림이 주인공인지를
 * 잃었다. **재료가 있다고 알려 주는 말이 따로 있어야 한다.**
 *
 * 이 줄을 넣고 다시 재니 장면·동작에 연출이 그대로 들어갔고, `forbidden` 은
 * 「인물 얼굴 변경, 모자 디자인 왜곡」으로 바뀌었다 — 우리 것을 지키는 쪽으로.
 */
describe("따라 만들 그림의 연출", () => {
  const prompt = buildPlanPrompt(input);

  it("연출이 재료라고 알려 준다", () => {
    expect(prompt).toContain("따라 만들 그림의 연출은 재료입니다");
  });

  /** 옮기되 정체성은 안 가져온다. 이것이 빠지면 레퍼런스의 얼굴이 샌다. */
  it("얼굴과 제품의 정체성은 빼라고 한다", () => {
    expect(prompt).toContain("사람의 얼굴과 제품의 정체성은 가져오지");
  });

  /**
   * **금지 칸에 적지 말라고 못 박는다.** 2026-09-18 에 기획이 정확히 그것을
   * 했다 — 따라 만들라고 붙인 그림의 연출을 `forbidden` 에 적었다.
   */
  it("그 연출을 금지 칸에 적지 말라고 한다", () => {
    expect(prompt).toContain("`forbidden` 에 적지 마세요");
  });
});
...
describe("글자를 지어냈을 때의 장면", () => {
  ...
  /**
   * **붙인 그림에 글자가 있으면 이 지시를 안 붙인다.**
   *
   * 이 지시의 까닭은 「어차피 지워질 글자」인데, 레퍼런스에 글자가 있으면
   * 지어낸 글자도 안 지워진다(`prompt.ts` 의 `글자를원한다`). 그대로 붙이면
   * 글자는 나가는데 어디에 놓을지를 아무도 안 적는다(2026-09-17 리뷰).
   */
  it("레퍼런스에 글자가 있으면 그 지시를 빼 준다", () => {
    const 있음 = buildPlanPrompt({ ...input, referenceHasText: true });

    expect(있음).not.toContain("타이포그래피·헤드라인·글자가 어디에 놓이는지");
    // 나머지 규칙은 그대로다 — 지우기와 안 붙이기는 다른 일이다.
    expect(있음).toContain("invented");
  });
