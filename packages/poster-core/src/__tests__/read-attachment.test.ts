...
describe("읽기 프롬프트", () => {
   ...
   * **한 사람이 한 덩이의 글이다.**
   *
   * 2026-09-18 첫 실측에서 세 장 중 둘이 여기서 깨졌다. 항목을 `people` 밑에
   * 들여써 뒀더니 모델이 하위 칸으로 읽고 **객체를 돌려줬다** —
   * `[{"자리":"가운데","머리":"긴 갈색"}]`. 틀은 글 목록을 받으므로 통째로
   * 떨어졌고, 그림 셋 중 둘의 인물 묘사가 사라졌다.
   */
  it("사람을 칸으로 쪼개지 말라고 한다", () => {
    expect(prompt).toContain("칸으로 쪼개지 마세요");
    // 보기를 준다. 모양을 말로 설명하는 것보다 세다.
    expect(prompt).toContain("보기:");
  });
