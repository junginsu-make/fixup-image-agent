...
export function buildPeoplePrompt(): string {
  return [
     ...
     * **「없음」을 적지 말라고 못 박는다.**
     *
     * 전에는 「없으면 적지 않습니다」가 **얼굴에 걸친 것에만** 붙어 있었다.
     * 모델은 나머지 항목에 성실히 「머리에 쓴 것 없음」을 적었고, 기획이
     * 그것을 그대로 옮겨(칸 설명이 그렇게 시킨다) 최종 프롬프트에 실렸다.
     *
     * **그 한 줄이 모자를 막았다**(2026-09-17 사용자 보고). 제품으로 따로 붙인
     * 모자가 결과에 안 나왔다. 인물 사진에 모자가 없는 것은 당연하다 — 모자는
     * 다른 장에 있다. 그런데 「이 사진에 없다」와 「그림에 없어야 한다」가
     * 그림 모델에게는 같은 말로 도착한다.
     */
