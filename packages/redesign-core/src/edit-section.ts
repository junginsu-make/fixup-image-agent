...
/**
 * Ported from `.refs/redesign-maker-10/src/app/api/edit-section/route.ts`.
 *
 * Only NextRequest JSON parsing and NextResponse serialization were removed.
 * Prompt strings, model IDs, API calls and error humanization are byte-for-byte
 ...
 *   - missing/invalid section image  -> 400 (RedesignError)
 *   - empty request text             -> 400 (RedesignError)
 *   - any other failure              -> 500; original outer catch wrapped the
 *                                       message with humanizeProviderError.
 *       => This function throws a plain Error for those; the adapter must do
 *          `{ error: humanizeEditError(err.message) }` at status 500.
 *
 * NOTE: edit-section has its OWN humanizeProviderError with shorter messages
 * (no request_id suffix), distinct from generate's. It is exported here as
 * `humanizeEditError` so the adapter applies the correct one for this route.
 */
...
export type EditSectionInput = {
  ...
  imageUrl?: string;
  request?: string;
  section?: { id?: string; name?: string; purpose?: string; source?: string } & Record<string, unknown>;
...
export async function editSection(input: EditSectionInput) {
  const provider: Provider = input.model === "google" ? "google" : "openai";
  const image = parseDataUrl(String(input.imageUrl || ""));
  ...
  if (!requestText.trim()) {
    throw new RedesignError("섹션 수정 요청사항을 입력해주세요.", 400);
  }

  const prompt = [
    // 수정 요청이 곧 사용자가 직접 친 지시다. 맨 앞과 맨 뒤에 두 번 넣는다 —
    // 긴 프롬프트에서 중간 문장은 힘을 잃는다(2026-09-04 실측).
    // 여기서는 requestText 가 비면 위에서 이미 400 으로 막히므로 항상 값이 있다.
    userInstructionHead(requestText),
...
export function humanizeEditError(message: string) {
