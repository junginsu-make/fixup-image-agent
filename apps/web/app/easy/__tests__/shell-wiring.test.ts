...
describe("구분선 둘", () => {
  ...
  /**
   * 2026-09-21 에는 「선이 안 보인다」는 말에 진하게 했고, 2026-09-22 에 「도로 흐리게, 대화와
   * 입력칸 사이 선과 같게」로 되돌렸다. 끌 수 있다는 것은 손잡이 아이콘이 알린다.
   */
  it("선 색이 대화·입력칸 구분선과 같은 테두리 색이다", () => {
    expect(handle).toContain("bg-border");
    expect(handle).not.toContain("bg-subtle-foreground/");
  });
});
