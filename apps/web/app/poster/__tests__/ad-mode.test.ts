...
const supported = AD_SPECS
  .filter((spec) => spec.format !== "png-alpha" && spec.supply !== "upload")
  .map((spec) => spec.id);
...
describe("제출해도 되는가", () => {
  ...
  it("못 만드는 규격이 섞여 있으면 못 한다", () => {
    const plan = adSubmitPlan(["google-rda-square", "kakao-bizboard"]);
    expect(plan.ready).toBe(false);
    expect(plan.reason).toContain("kakao-bizboard");
  });

  it("막힌 까닭을 그대로 말한다 — 기다리면 되는지 다른 길인지 알아야 한다", () => {
    expect(adSubmitPlan(["kakao-bizboard"]).reason).toMatch(/투명/);
  });
...
describe("몇 장을 만드는가", () => {
  ...
  it("못 하는 상태에서는 만들 것을 세지 않는다", () => {
    expect(adSubmitPlan(["google-rda-square", "kakao-bizboard"]).masters).toEqual([]);
  });
});
