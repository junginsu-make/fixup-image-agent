 ...
 *
 * **세 칸에 한 화면**이다. 이 화면은 섹션이 크고 사이가 넓어서 브라우저 기본
 * (한 칸에 100px 안팎)으로는 「거의 안 움직인다」로 느껴진다.
 *
 * 어느 마우스, 어느 모니터든 한 칸이 화면의 같은 만큼을 지나간다 — 칸을 세고
 * 화면에 비례한 거리를 준다.
 *
 * ── 값을 네 번 고쳤다 ────────────────────────────────────────────
 * 남겨 두는 이유는 다음 사람이 같은 길을 다시 걷지 않게 하려는 것이다.
 *
 *   `deltaY × 2.4`   **마우스마다 갈렸다.** 크롬은 한 칸에 100 을 보내는데
 *                    윈도의 「한 번에 스크롤할 줄 수」를 1 로 둔 마우스는 33 쯤을
 *                    보낸다. 그 사람은 79px 밖에 안 내려갔다
 *   고정 260px       **모니터마다 갈렸다.** 노트북 3분의 1, 큰 모니터 8분의 1
 *   0.4              한 번에 너무 멀리 갔다(보이기 전에 되돌림)
 *   0.15             「아직도 조금씩 내려간다」 — 운영자가 실제로 확인
 *
 * 값을 바꿀 일이 생기면 여기 한 줄이다.
 */
export const WHEEL_SCREEN_RATIO = 0.33;
...
/**
 * 앞 휠에서 이보다 촘촘히 오면 **흐르는 입력**일 수 있다.
 *
...
/**
 * 촘촘히 와도 **이만큼 크면 마우스로 본다.**
 *
 * 처음에는 「촘촘하면 무조건 흐르는 입력」으로 뒀다. 그런데 부드러운 스크롤을
 * 지원하는 마우스·드라이버는 **한 칸을 여러 이벤트로 쪼개 빠르게 보낸다.**
 * 그러면 첫 이벤트만 우리 손을 타고 나머지는 브라우저 기본으로 빠져, 굴려도
 * 조금씩 내려가는 것처럼 느껴진다.
 *
 * 크기로 한 번 더 가른다. 트랙패드의 잔 값은 이보다 훨씬 작다.
 */
export const WHEEL_STREAM_MAX_DELTA = 40;
...
export function boostedWheel(event: {
  ...
  // 흐르는 입력(트랙패드)이다 — **작은 값이 촘촘히** 올 때만 그렇게 본다.
  // 큰 값이 촘촘히 오는 것은 한 칸을 쪼개 보내는 마우스다. 그건 받아 준다.
  if (event.sinceLast < WHEEL_STREAM_MS && Math.abs(event.deltaY) < WHEEL_STREAM_MAX_DELTA) {
    return 0;
  }
