...
export function CreditWallet({ usage }: { usage: UsageSummary }) {
  return <section className="space-y-4 rounded-xl border bg-card p-5" aria-label="크레딧 잔액">
    <h2 className="text-lg font-semibold">내 크레딧</h2>
    <p><strong className="text-3xl">{usage.remaining.toLocaleString()}</strong> 크레딧 사용 가능</p>
    <p className="text-sm text-muted-foreground">일반 이미지·카드 1장 = 1크레딧 · 인쇄용 600만 픽셀 이상 = 2크레딧. 기획·분석은 무료입니다.</p>
    ...
    <p className="text-sm">처리 중 {usage.reserved}크레딧 · 이번 달 사용 {usage.used}크레딧</p>
