 ...
 *
 * **`POSTER_RATIOS` 를 참조하지 않는다.** 그 배열에 기대면 기존 목록이 바뀔 때
 * 광고가 조용히 따라 바뀐다. 광고는 자기 값을 들고 `match-source` 로 넘긴다
 * (설계 §4.2).
 *
 * **초판에는 「값이 같은 항목이 둘 있다」(`ad-4x5`·`ad-9x16`)고 적혀 있었는데
 * 2026-09-23 에 거짓이 됐다.** 그날 `POSTER_RATIOS` 의 4:5·9:16 이 광고 규격을
 * 덮으려고 1216×1520·1296×2304 로 커졌고, 여기 값은 그대로다. **분리가 뜻대로
 * 작동한 것이다** — 한쪽이 움직여도 다른 쪽이 따라가지 않았다. 지금 겹치는
 * 값은 하나도 없으므로, 「어차피 같으니 파생시키자」는 판단을 하면 안 된다.
 */
