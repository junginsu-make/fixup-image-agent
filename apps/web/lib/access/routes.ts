 ...
 *
 * **단 사이드바는 예외다.** 실제 메뉴는 `packages/ui` 에 손으로 적혀 있고
 * (`components/app-shell.tsx` 의 `navGroups`) 이 등록부에서 파생되지 않는다.
 * 위의 「셋」은 **역할 규칙**을 보는 셋(미들웨어·페이지 문지기·관리자 배지)
 * 이야기다. 화면을 늘리거나 끌 때는 저 파일도 함께 봐야 한다 —
 * `__tests__/routes.test.ts` 가 둘이 어긋나는지 지킨다.
 *
 * ── 아직 코드에 두는 이유 ─────────────────────────────────────────
 *
