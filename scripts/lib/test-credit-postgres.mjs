...
export async function testPostgres() {
  ...
  const dir = await mkdtemp(path.join(base, 'fixup-credit-test-'));
  ...
  async function close() {
    ...
    if (path.dirname(actual) !== base || !path.basename(actual).startsWith('fixup-credit-test-')) throw new Error('Refusing to remove unexpected test cluster');
